...
How could we convert it to handle color images? Please include your code. \\
...
(a) Please time your code execution!
...
\emph{Don't assume that the time taken for one image $\times1000$ will equal $1000$ image computations, as single short tasks on multitasking computers often take variable time. But, running the code on 1000 images might take 45 minutes or more! Instead, compute the time on a smaller number, say, 10 images, and compute the average time.}

(b) You might find the code is too slow to handle 1000 images---why is it slow? Let's speed it up. Using what we learned in our Python tutorial, write code for a sped-up version.
